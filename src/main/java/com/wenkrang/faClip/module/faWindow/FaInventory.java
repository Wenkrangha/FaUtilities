...
public class FaInventory {
    private FaWindowInstance faWindowInstance;
    ...
    private Map<Integer, ItemStack> override = new HashMap<>();

    public boolean lock = true;

    private static void setMoveable(FaInventory target,FaInventory src) {
    ...
    public static void clone(FaInventory target, FaInventory src) {
        target.faWindowInstance = src.faWindowInstance;
        ...
        target.events = new HashMap<>(src.events);
        target.design(src.design);

        target.define = new HashMap<>();
        for (Map.Entry<String, ItemStack> entry : src.define.entrySet()) {
            target.define.put(entry.getKey(), entry.getValue() == null ? null : entry.getValue().clone());
        }

        target.note.putAll(src.note);

        setMoveable(target, src);

        target.override = new HashMap<>(src.override);
    ...
    public FaInventory getTemplate() {
        return faWindowInstance.getFaInventory(template);
    }
